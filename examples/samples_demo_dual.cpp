...
public:
    StandardAudioPlayer() : stream_(nullptr), current_position_(0), is_playing_(false) {}
    ...
    void play() {
        ...
        is_playing_ = true;
        current_position_ = 0;
        std::cout << "  🎵 Playing audio with STANDARD system..." << std::endl;
        ...
        is_playing_ = false;
        std::cout << "  ✅ Standard audio playback completed" << std::endl;
    }
...
private:
    static int standardAudioCallback(const void* inputBuffer, void* outputBuffer,
                                   unsigned long framesPerBuffer,
                                   const PaStreamCallbackTimeInfo* timeInfo,
                                   PaStreamCallbackFlags statusFlags,
                                   void* userData) {
        StandardAudioPlayer* player = static_cast<StandardAudioPlayer*>(userData);
...
class AnantaSoundAudioPlayer {
private:
    AnantaSoundCore core_;
    StandardAudioPlayer audio_player_;
    
public:
    AnantaSoundAudioPlayer() : core_(5.0, 3.0) {}
    ...
    void playSample(const std::string& sample_name, const std::string& filepath) {
        std::cout << "  🎵 Playing audio with ANANTA SOUND system..." << std::endl;
        ...
        std::cout << "  ✅ AnantaSound audio playback completed" << std::endl;
    }
...
class DualModeSamplesPlayer {
private:
    StandardAudioPlayer standard_player_;
    AnantaSoundAudioPlayer ananta_player_;
...
public:
    ...
    bool initialize() {
        if (!standard_player_.initialize()) {
            std::cerr << "Failed to initialize standard audio player" << std::endl;
            return false;
        }
        std::cout << "Standard audio player initialized successfully" << std::endl;
        
        if (!ananta_player_.initialize()) {
            std::cerr << "Failed to initialize anAntaSound audio player" << std::endl;
            return false;
        }
        std::cout << "anAntaSound audio player initialized successfully" << std::endl;
        
        return true;
    }
    
    void playSampleDualMode(const std::string& sample_name) {
        std::cout << "\n🎵 Playing: " << sample_name << std::endl;
        std::cout << "=====================================" << std::endl;
        
        std::string filepath = samples_dir_ + "/" + sample_name;
        
        // Сначала стандартное воспроизведение
        std::cout << "\n🔊 MODE 1: Standard Audio Playback" << std::endl;
        if (standard_player_.loadAudioFile(filepath)) {
            standard_player_.play();
        }
        
        std::cout << "\n⏸️  Pausing between modes..." << std::endl;
        std::this_thread::sleep_for(std::chrono::seconds(2));
        
        // Затем anAntaSound воспроизведение
        std::cout << "\n🌟 MODE 2: anAntaSound Quantum Audio Processing" << std::endl;
        ananta_player_.playSample(sample_name, filepath);
    }
    
    void playAllSamplesDualMode() {
        if (sample_files_.empty()) {
            std::cout << "No samples to play" << std::endl;
            return;
        }
        
        std::cout << "\n🎼 Starting Dual-Mode anAntaSound Samples Demo" << std::endl;
        std::cout << "=============================================" << std::endl;
        std::cout << "Each sample will be played in TWO modes:" << std::endl;
        std::cout << "1. Standard audio playback" << std::endl;
        std::cout << "2. anAntaSound quantum acoustic processing" << std::endl;
        
        for (const auto& sample : sample_files_) {
            playSampleDualMode(sample);
        ...
        std::cout << "\n🎉 All samples played in dual mode successfully!" << std::endl;
    }
    ...
    void interactiveMode() {
        ...
        std::cout << "\n🎮 Interactive Dual-Mode - Choose sample to play:" << std::endl;
        ...
        while (true) {
            ...
            try {
                ...
                if (index < sample_files_.size()) {
                    playSampleDualMode(sample_files_[index]);
                } else {
    ...
    void shutdown() {
        ananta_player_.shutdown();
        std::cout << "All audio systems shutdown" << std::endl;
    }
};

int main() {
    std::cout << "anAntaSound Dual-Mode Samples Demo Application" << std::endl;
    std::cout << "Version: " << getVersion() << std::endl;
    std::cout << "Build Info: " << getBuildInfo() << std::endl;
    
    DualModeSamplesPlayer player;
    ...
    std::cout << "1. Play all samples in DUAL MODE (Standard + anAntaSound)" << std::endl;
    std::cout << "2. Interactive dual-mode (choose samples)" << std::endl;
    ...
    if (choice == "1") {
        player.playAllSamplesDualMode();
    } else if (choice == "2") {
        player.interactiveMode();
    } else {
        std::cout << "Invalid choice, playing all samples in dual mode" << std::endl;
        player.playAllSamplesDualMode();
    }
